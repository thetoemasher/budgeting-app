...
module.exports = {
    ...
    getOneMonth: async (req, res) => {
        try {
            ...
            let monthObj = {}
            if(month_id) {
                monthObj = await db.months.get_month_by_id(month_id)
            } else {
                monthObj = await db.months.get_month_by_month_and_year([month, year, user_id])
            }
            month_id = monthObj[0].month_id
            ...
            console.log('Error in getOneMonth method in months_controller', err)
            res.status(500).send(err)
        }
    }
}